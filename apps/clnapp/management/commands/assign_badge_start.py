...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        users_with_star_badge = UserBadge.objects.filter(badge__name=BADGE_START, user=OuterRef("author")).values("user")

        model3ds = Model3d.objects.annotate(has_star_badge=Subquery(users_with_star_badge)).filter(
            nb_views__gte=CONDITION_START, has_star_badge=None
        )
